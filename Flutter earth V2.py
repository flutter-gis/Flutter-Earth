...
import base64
import socket
import random # Added for random splash screen selection
...
# --- Splash Screen Drawing Functions (declarations needed before use) ---
# These functions are defined later in the script but need to be known for SPLASH_SCREEN_OPTIONS
def draw_planets_splash(painter, width, height): pass # Keep Planets
def draw_flowers_splash(painter, width, height): pass # Commented out
# def draw_desert_splash(painter, width, height): pass # Commented out
...
def check_and_reinstall_all_packages():
    """Attempts to reinstall all required packages.
    This will run on every script start and can take a significant amount of time.
    """
    required_packages = {
        # Format: "import_name": "package_name_for_pip==version" or "package_name_for_pip"
        "requests": "requests",
        "rasterio": "rasterio",
        "numpy": "numpy==1.26.4",  # Pinning numpy to a widely compatible version
        ...
        "webview": "pywebview",
        "shapely": "shapely",
        "matplotlib": "matplotlib==3.8.4", # Pinning matplotlib
        "qtpy": "qtpy",
        "PyQt5": "PyQt5", # Consider PyQt5==5.15.10 if issues persist
        "pyqtgraph": "pyqtgraph",
        "PyQtWebEngine": "PyQtWebEngine", # Consider PyQtWebEngine==5.15.6
        "shapefile": "pyshp",
        "overpass": "overpass"
    }
   
    # Get all package install names (with versions if specified) to attempt reinstallation
    packages_to_reinstall = list(required_packages.values())
   
    logging.info(f"Attempting to reinstall all {len(packages_to_reinstall)} required packages: {', '.join(packages_to_reinstall)}")
    print(f"\nFlutter Earth: Attempting to reinstall all {len(packages_to_reinstall)} required packages.")
    print("This process will run every time the application starts and may take a significant amount of time.")
    print("Please be patient...")
       
    pip_ok = True
    for pkg_name in packages_to_reinstall:
        try:
            print(f"Reinstalling {pkg_name}...")
            # Command to force reinstall. If version is specified, it will install that version.
            # If no version specified, --upgrade will take it to the latest.
            install_command = [sys.executable, "-m", "pip", "install", "--force-reinstall"]
            if "==" not in pkg_name: # If no version is pinned, add --upgrade
                install_command.append("--upgrade")
            install_command.append(pkg_name)
            
            # Add --no-cache-dir for specific packages to ensure fresh download
            # Check against the base package name if version is pinned
            base_pkg_name_for_check = pkg_name.split("==")[0]
            if base_pkg_name_for_check == "earthengine-api" or "google-api" in base_pkg_name_for_check or "google-auth" in base_pkg_name_for_check:
                if "--no-cache-dir" not in install_command:
                    install_command.append("--no-cache-dir")
            
            subprocess.check_call(install_command)
            logging.info(f"Successfully reinstalled {pkg_name}.")
            print(f"Successfully reinstalled {pkg_name}.")
        except subprocess.CalledProcessError as e:
            logging.error(f"Failed to reinstall {pkg_name}. Error: {e}")
            print(f"ERROR: Failed to reinstall {pkg_name}. The script will attempt to continue, but might encounter issues.")
            # Continue trying to reinstall other packages even if one fails.
            # pip_ok = False # You could set this to False to indicate an overall failure.
        except FileNotFoundError:
            logging.error("'pip' not found. Ensure Python & pip are in PATH.")
            print("CRITICAL ERROR: 'pip' not found. Cannot reinstall packages. Flutter Earth cannot continue.")
            pip_ok = False
            break # Stop if pip itself is not found.
       
    print("Package reinstallation attempts complete.\n")
    if not pip_ok: # If pip itself was not found.
        return False
    
    logging.info("All required packages have been processed for reinstallation.")
    return True

if not check_and_reinstall_all_packages():
    print("Critical error during package reinstallation process. Flutter Earth may not run correctly or will exit.")
    # Consider exiting if critical packages are missing, though the script tries to proceed.
    # If pip was not found, it's a good idea to exit.
    if "'pip' not found" in open(SCRIPT_CONSOLE_LOG_PATH).read()[-200:]: # Quick check in log
        sys.exit("Exiting due to missing 'pip'.")
...
SPLASH_SCREEN_STYLES = {
    "Planets": draw_planets_splash,
    "Flowers": draw_flowers_splash, # Commented out
    # "Desert": draw_desert_splash, # Commented out
    "Random": "Random" # Random will now pick from available options (Planets, Flowers)
}
DEFAULT_SPLASH_SCREEN_STYLE = "Random"
...
FLUTTERSHY_TEXTS = { # These are for the QtPy part
    ...
    "vector_download_action_label": "🗺️ Vector Data", # New for menu action
    ...
    "appearance_menu_text": "&Appearance ✨", # New for Appearance menu
    "splash_screen_style_submenu_text": "Splash Screen Style 🌠", # New for Splash Screen submenu
    "vector_download_tab_title": "🗺️ Vector Data Download",
    "vector_source_label": "Overpass Query / URL:", # More concise
    "vector_type_label": "Source Type:",
    "vector_output_dir_label": "Save Vector To:", # More concise
    ...
    "vector_status_fail": "Vector download failed: {error}", # More concise
...
APP_CONFIG = {
    ...
    'splash_screen_style': DEFAULT_SPLASH_SCREEN_STYLE # New entry for splash screen
}

ALL_SAMPLE_CONFIGS = {
    "AMAZON_SAMPLE": {
        "area_name": "AMAZON_SAMPLE",
        "bbox": [-60.25, -3.25, -60.0, -3.0], # minLon, minLat, maxLon, maxLat (near Manaus, 0.25x0.25 deg)
        "year": 2023,
        "month": 7, # A single recent month
        "sensor": "SENTINEL2",
        "target_resolution": 30, # meters, fairly coarse for a quick sample
        "output_subdir": "flutter_earth_sample_data",
    },
    "HAIFA_PORT_SAMPLE": {
        "area_name": "HAIFA_PORT_SAMPLE",
        "bbox": [34.98, 32.78, 35.03, 32.83], # Haifa Port, Israel
        "year": 2023,
        "month": 7,
        "sensor": "SENTINEL2",
    ...
    "ARCTIC_SVALBARD_SAMPLE": {
        "area_name": "ARCTIC_SVALBARD_SAMPLE",
        "bbox": [15.0, 78.5, 16.0, 79.0], # Svalbard, Arctic
        "year": 2023,
        "month": 8,
        "sensor": "SENTINEL2",
    ...
    "TOKYO_URBAN_SAMPLE": {
        "area_name": "TOKYO_URBAN_SAMPLE",
        "bbox": [139.73, 35.67, 139.78, 35.72], # Tokyo, Japan (central area)
        "year": 2023,
        "month": 10,
        "sensor": "SENTINEL2",
        "target_resolution": 10,
    ...
    "SAHARA_ALGERIA_DUNES_SAMPLE": {
        "area_name": "SAHARA_ALGERIA_DUNES_SAMPLE",
        "bbox": [2.0, 27.0, 2.2, 27.2], # Sahara Desert, Algeria (dunes)
        "year": 2023,
        "month": 4,
        "sensor": "SENTINEL2",
...
DEFAULT_SAMPLE_KEY = "AMAZON_SAMPLE" # Change this key to test other default samples on startup
...
# ==============================================
#        SCRIPT DIRECTORY CONSOLE LOG
# ==============================================
# Log console output to a file next to the script, clearing on each run.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__)) if '__file__' in locals() else os.getcwd()
SCRIPT_CONSOLE_LOG_PATH = os.path.join(SCRIPT_DIR, "flutter_earth_console.log")

script_console_file_handler = logging.FileHandler(SCRIPT_CONSOLE_LOG_PATH, mode='w', encoding='utf-8')
script_console_file_handler.setFormatter(LOG_FORMATTER)
ROOT_LOGGER.addHandler(script_console_file_handler)

logging.info(f"Console output also being logged to: {SCRIPT_CONSOLE_LOG_PATH}")

logging.info("Initial logging configured. stdout and stderr redirected to queue.")

def setup_main_app_logging(output_dir_for_log):
    ...
    # Use 'a' for append mode so logs from multiple runs (if not cleared) accumulate,
    # or 'w' for overwrite if a fresh log is desired each time. 'a' is safer for history.
    file_handler = logging.FileHandler(log_file_path, mode='a', encoding='utf-8')
    file_handler.setFormatter(LOG_FORMATTER)
    ROOT_LOGGER.addHandler(file_handler)
    logging.info(f"Main application logging configured. Output to: {log_file_path}")
...
if __name__ == "__main__":
    ...
    class VectorDownloadTab(QtWidgets.QWidget):
        ...
        def _init_ui(self):
            ...
            # Main form layout for the tab - Initialize this first!
            form_layout = QtWidgets.QFormLayout()
            form_layout.setSpacing(10)

            # Data Source Input
            self.source_input = QtWidgets.QLineEdit()
            self.source_input.setPlaceholderText("Enter Overpass QL query or direct download URL for vector data.")
            ...
            self.vector_aoi_input.setToolTip("Define the AOI for Overpass queries. Can be a BBOX string or GeoJSON polygon coordinates.") # Add tooltip
            ...
            self.overpass_options_group = QtWidgets.QGroupBox("Overpass API Query Options") # More specific
            self.overpass_options_layout = QtWidgets.QVBoxLayout(self.overpass_options_group)

            # Radio buttons for Overpass query mode
            # Tooltips for radio buttons could be added if needed, but text seems clear.
            # self.rb_predefined_features.setToolTip("Use pre-configured queries for common features like roads or buildings.")
            # self.rb_custom_query.setToolTip("Enter your own Overpass QL query.")
            ...
            self.predefined_features_group = QtWidgets.QGroupBox("Select OSM Features (for Predefined Query)") # More descriptive
            ...
            self.cb_osm_roads.setToolTip("Download road networks from OpenStreetMap.") # Add tooltip
            self.cb_osm_buildings = QtWidgets.QCheckBox("Buildings (building=*)")
            self.cb_osm_buildings.setToolTip("Download building footprints from OpenStreetMap.") # Add tooltip
            self.cb_osm_waterways = QtWidgets.QCheckBox("Waterways (waterway=*)")
            self.cb_osm_waterways.setToolTip("Download rivers, canals, and other waterways from OpenStreetMap.") # Add tooltip
            ...
            self.output_dir_input_vd.setPlaceholderText("Choose folder to save downloaded vector files...") # More user-friendly
            ...
            self.output_format_combo.setToolTip("Select the desired output file format for the vector data.") # Add tooltip
            ...
            # Start Download Button (Text updated via FLUTTERSHY_TEXTS)
            ...
            self.source_input.setEnabled(True) # Ensure custom query input is enabled by default
            self._on_overpass_query_mode_changed() # Set initial UI state based on default radio button
        ...
        def _open_map_for_vector_aoi(self):
                ...
                if current_aoi_text:
                    try:
                        ...
                        if isinstance(parsed_coords, list) and \
                           all(isinstance(p, list) and len(p) == 2 and all(isinstance(coord, (float, int)) for coord in p) for p in parsed_coords) and \
                           len(parsed_coords) >= 3:
            ...
            except Exception as e_map_dialog_creation:
                logging.error(f"Failed to create or execute MapSelectionDialog for vector AOI: {e_map_dialog_creation}", exc_info=True)
                QtWidgets.QMessageBox.critical(self, "Map Dialog Error",
                                               f"Could not open the map selection dialog.\nError: {str(e_map_dialog_creation)[:200]}\n\n"
                                               "Please ensure PyQtWebEngine is correctly installed and functional. Check logs for more details.")
        ...
        def _on_overpass_query_mode_changed(self):
            # AOI input is always enabled for Overpass
            self.vector_aoi_input.setEnabled(True)
            self.vector_aoi_map_button.setEnabled(True)
            self.vector_aoi_input.setPlaceholderText("AOI (e.g., minLon,minLat,maxLon,maxLat or [[lon,lat],...])")
            ...
            if is_custom_mode: # Custom Query Mode
                self.source_input.setPlaceholderText("Enter your full Overpass QL query here (use {{bbox}} for AOI)...")
                self.source_input.setToolTip("Enter a custom Overpass QL query. Use {{bbox}} as a placeholder for the AOI.")
            else: # Predefined Features Mode
                self.source_input.setPlaceholderText("Query will be auto-generated for predefined features.")
                self.source_input.setToolTip("Query will be automatically generated based on selected predefined features and the AOI.")
        ...
        def _start_vector_download_wrapper(self):
            ...
            except Exception as e_perform_dl: # More specific exception variable name
                logging.error(f"Vector download GUI error: {e_perform_dl}", exc_info=True)
                QtWidgets.QMessageBox.critical(self, "Download Error", f"An unexpected error occurred: {e_perform_dl}")
                status_bar_qt.showMessage(FLUTTERSHY_TEXTS.get("vector_status_fail", "Vector download failed: {error}").format(error=str(e_perform_dl)))


        def _perform_vector_download(self):
            # data_source_text is no longer a generic source, it's the custom query if applicable
            output_dir = self.output_dir_input_vd.text().strip()
            output_format = self.output_format_combo.currentText()  # e.g., "GeoJSON"
            status_bar_ref = main_window.statusBar() # Get a reference to the main window's status bar

            if not output_dir:
                QtWidgets.QMessageBox.warning(self, "Output Directory Missing", "Please specify an output directory for vector data.")
                return
            os.makedirs(output_dir, exist_ok=True)

            overpass_query = ""
            # api_call_build_param = False # This variable seems unused with the 'overpass' library's build=False approach
            source_type_for_log = "Overpass API"

            aoi_param_for_overpass = "" # This will be (S,W,N,E) or (poly:"...")
            ...
            if self.rb_custom_query.isChecked():
                    ...
                    # Parse AOI if {{bbox}} is in the query, or if AOI field is not empty (user might provide AOI without {{bbox}})
                    if "{{bbox}}" in custom_query_text:
                        if not aoi_text: # Check the common aoi_text variable
                        ...
                        try: # Try parsing as BBOX: "minLon,minLat,maxLon,maxLat"
                            ...
                            try: # Try parsing as Polygon JSON: "[[lon,lat], [lon,lat], ...]"
                                poly_coords_lon_lat = json.loads(aoi_text)
                                ...
                                QtWidgets.QMessageBox.critical(self, "Invalid AOI", f"Could not parse AOI for custom query: {aoi_text}. Error: {e_poly}")
                                return
                    overpass_query = custom_query_text.replace("{{bbox}}", aoi_param_for_overpass)
                    feature_name_parts = ["custom_query"] # For filename

            elif self.rb_predefined_features.isChecked():
                    feature_name_parts = [] # For filename
                    if not aoi_text: # Check the common aoi_text variable
                        QtWidgets.QMessageBox.warning(self, "AOI Missing", FLUTTERSHY_TEXTS.get("vector_aoi_missing_error", "AOI is required for predefined Overpass API queries."))
                        return
                    # Parse AOI for predefined features (similar to custom query's {{bbox}} parsing)
                    try:
                        coords = [float(c.strip()) for c in aoi_text.split(',')]
                        if len(coords) == 4:
                        ...
                        try: # Try parsing as Polygon JSON: "[[lon,lat], [lon,lat], ...]"
                            ...
                            QtWidgets.QMessageBox.critical(self, "Invalid AOI", f"Could not parse AOI for predefined features: {aoi_text}. Error: {e_poly}")
                            return

                    selected_feature_queries_fragments = []
                    # Define query fragments for each predefined feature type
                    # These fragments expect aoi_param_for_overpass to be already defined
                    # Using (._;>;); to fetch complete geometries including nodes.
                    query_templates = {
                        "roads":     f"way[\"highway\"]{aoi_param_for_overpass};(._;>;);",
                        "buildings": f"(way[\"building\"]{aoi_param_for_overpass}; relation[\"building\"]{aoi_param_for_overpass};);(._;>;);",
                        "waterways": f"(way[\"waterway\"]{aoi_param_for_overpass}; relation[\"waterway\"]{aoi_param_for_overpass};);(._;>;);",
                    }

                    if self.cb_osm_roads.isChecked():
                        selected_feature_queries_fragments.append(query_templates["roads"])
                        feature_name_parts.append("roads")
                    if self.cb_osm_buildings.isChecked():
                        selected_feature_queries_fragments.append(query_templates["buildings"])
                        feature_name_parts.append("buildings")
                    if self.cb_osm_waterways.isChecked():
                        selected_feature_queries_fragments.append(query_templates["waterways"])
                        feature_name_parts.append("waterways")

                    if not selected_feature_queries_fragments:
                        QtWidgets.QMessageBox.warning(self, "No Features Selected", "Please select at least one predefined OSM feature type.")
                        return

                    # Combine selected feature queries into a single Overpass query
                    combined_feature_queries = "\n  ".join(selected_feature_queries_fragments)
                    overpass_query = f"""
[out:json][timeout:60];
(
  {combined_feature_queries}
);
out geom;""" # 'out geom;' is often sufficient for GeoJSON conversion by the 'overpass' library
                ...
                status_bar_ref.showMessage(FLUTTERSHY_TEXTS.get("vector_status_fetching", "Fetching...").format(source="Overpass API"))
                logging.info(f"Overpass Query (using 'overpass' library): {overpass_query[:200]}...")

                logging.debug(f"Full Overpass Query being sent:\n{overpass_query}")
                ...
                try:
                    ...
                    # The 'overpass' library's .get() method with responseformat="geojson"
                    # expects the full query string when build=False.
                    # When build=True, it expects parameters to build the query.
                    # Since we are building the full query string ourselves (especially for predefined features),
                    # we should use build=False.
                    logging.debug(f"Calling Overpass API.get with build=False, verbosity='geom', responseformat='geojson'")
                    geojson_fc = api.get(overpass_query, build=False, verbosity='geom', responseformat="geojson")
                    logging.info(f"Overpass API call returned. Type of result: {type(geojson_fc)}")

                    # --- Improved Feedback for No Features ---
                    if isinstance(geojson_fc, dict) and geojson_fc.get("type") == "FeatureCollection":
                        num_features = len(geojson_fc.get('features', []))
                        logging.info(f"Number of features received from Overpass: {num_features}")
                        if num_features == 0:
                            logging.warning("Overpass query returned 0 features. The AOI might not contain the requested data or the query might be too restrictive.")
                            QtWidgets.QMessageBox.information(self, "No Features Found",
                                                              f"The Overpass query ran successfully but returned 0 features for the specified Area of Interest.\n\n"
                                                              "This might mean there is no data of the selected type in this area, or your query/AOI is incorrect.\n\n"
                                                              "Check the logs for the query sent.") # noqa: E501
                            status_bar_ref.showMessage(FLUTTERSHY_TEXTS.get("vector_status_fail", "Failed: No features found."))
                            return # Exit download process if no features
                    else:
                        logging.warning(f"Unexpected result format from Overpass API. Expected GeoJSON FeatureCollection dict. Got: {str(geojson_fc)[:200]}")
                        QtWidgets.QMessageBox.critical(self, "Overpass Data Error",
                                                       f"Received unexpected data format from Overpass API. Expected GeoJSON FeatureCollection.\n\n"
                                                       f"Received type: {type(geojson_fc)}. Check logs for details.")
                        status_bar_ref.showMessage(FLUTTERSHY_TEXTS.get("vector_status_fail", "Failed: Unexpected data format."))
                        return # Exit download process if data format is wrong
                   
                    status_bar_ref.showMessage(FLUTTERSHY_TEXTS.get("vector_status_processing", "Processing..."))
                    ...
                    base_filename = f"osm_{filename_feature_part}_{datetime.now().strftime('%Y%m%d%H%M%S')}"

                    # --- Implement Shapefile Output ---
                    if output_format == "GeoJSON":
                        ...
                        logging.info(f"Saved Overpass data as GeoJSON: {output_filepath}")
                        status_bar_ref.showMessage(FLUTTERSHY_TEXTS.get("vector_status_success", "Saved: {filename}").format(filename=os.path.basename(output_filepath)))
                        ...
                        for feature in geojson_fc.get('features', []):
                            ...
                            if geom and geom.get('type') in features_by_type:
                                features_by_type[geom['type']].append(feature)
                            elif geom and geom.get('type') == 'MultiPolygon': # Handle MultiPolygon
                                features_by_type['Polygon'].append(feature) # Treat MultiPolygon as Polygon for saving
                            # Add other types like MultiLineString, MultiPoint if needed

                        saved_files = []
                        for geom_type, features_list in features_by_type.items():
                            if features_list:
                                # Determine shapefile type based on GeoJSON type
                                shp_type = None
                                ...
                                if shp_type is not None:
                                    output_filepath_shp = os.path.join(output_dir, f"{base_filename}_{geom_type.lower()}s.shp")
                                    status_bar_qt.showMessage(FLUTTERSHY_TEXTS.get("vector_status_saving", "Saving...").format(format=f"{geom_type} Shapefile", filename=os.path.basename(output_filepath_shp)))
                                    self._save_features_to_shapefile(features_list, shp_type, output_filepath_shp)
                                    saved_files.append(os.path.basename(output_filepath_shp))
                                else:
                                    logging.warning(f"Skipping saving {geom_type} features: Unsupported shapefile type.")

                        status_bar_ref.showMessage(FLUTTERSHY_TEXTS.get("vector_status_success", "Saved: {filename}").format(filename=", ".join(saved_files) if saved_files else "No Shapefiles saved."))
                        logging.info(f"Finished saving Overpass data as Shapefile(s): {', '.join(saved_files)}")
                    ...
                    QtWidgets.QMessageBox.critical(self, "Overpass Library Error", f"Failed to fetch/process data using Overpass library: {str(e_op_lib)[:200]}")
                    status_bar_ref.showMessage(FLUTTERSHY_TEXTS.get("vector_status_fail", "Failed: {error}").format(error=str(e_op_lib)[:100]))
                except Exception as e_proc:
                    logging.error(f"Error processing Overpass data: {e_proc}", exc_info=True)
                    QtWidgets.QMessageBox.critical(self, "Processing Error", f"Failed to process data: {str(e_proc)[:200]}")
                    status_bar_ref.showMessage(FLUTTERSHY_TEXTS.get("vector_status_fail", "Failed: {error}").format(error=str(e_proc)[:100]))


        def _save_features_to_shapefile(self, features, shape_type, output_filepath):
            """Saves a list of GeoJSON features of a single geometry type to a shapefile."""
            ...
            try:
                ...
                with shapefile.Writer(output_filepath, shapeType=shape_type) as w:
                    # Define fields based on common OSM tags and GeoJSON properties
                    # pyshp requires field names to be <= 10 characters
                    w.field('osm_id', 'N', 15) # OSM ID as Number
                    w.field('name', 'C', 100) # Name as Character string
                    w.field('primary_tag', 'C', 50) # Store the key of the primary tag (e.g., 'highway', 'building')
                    w.field('tag_value', 'C', 100) # Store the value of the primary tag (e.g., 'residential', 'yes')
                    # Add more fields if needed, but keep it simple for common tags
                    ...
                    for feature in features:
                        ...
                        osm_id = props.get('osm_id', -1)
                        name = props.get('name', '')

                        # Find a "primary" tag (heuristic: first tag that's not osm_id, name, type etc.)
                        primary_tag_key = ''
                        primary_tag_value = ''
                        for key, value in props.items():
                            if key not in ['osm_id', 'osm_type', 'name']:
                                primary_tag_key = key
                                primary_tag_value = str(value) # Ensure value is string
                                break # Take the first one found

                        # Write record
                        w.record(osm_id, name, primary_tag_key, primary_tag_value)

                        # Write geometry - pyshp expects list of points for Point/PolyLine, list of lists of points for Polygon
                        coords_raw = geom.get('coordinates')
                        if shape_type == shapefile.POINT:
                            w.point(*coords_raw) # Expects [x, y]
                        elif shape_type in [shapefile.POLYLINE, shapefile.POLYGON]:
                            w.poly(coords_raw) # Expects [[x,y],...] for LineString, [[[x,y],...]] for Polygon

                logging.info(f"Shapefile saved: {output_filepath}")
            except Exception as e:
                logging.error(f"Error saving shapefile {output_filepath}: {e}", exc_info=True)
                raise # Re-raise to be caught by the caller (_perform_vector_download)
    ...
    # --- Splash Screen Drawing Functions ---
    def draw_planets_splash(painter, width, height):
        # Background: Dark space
        # Ensure painter is valid
        if not painter or not painter.isActive():
            logging.error("draw_planets_splash: Painter is not active or invalid.")
            return
        space_gradient = QtGui.QLinearGradient(0, 0, 0, height)
        space_gradient.setColorAt(0, QtGui.QColor("#000020")) # Deep dark blue
        space_gradient.setColorAt(1, QtGui.QColor("#000000")) # Black
        painter.fillRect(0, 0, width, height, space_gradient)

        # Stars
        painter.setPen(Qt.NoPen)
        for _ in range(400): # More stars for a larger canvas
            star_x = random.randint(0, width)
            star_y = random.randint(0, height)
            star_size = random.uniform(0.5, 2.5)
            star_color = QtGui.QColor(random.randint(200, 255), random.randint(200, 255), random.randint(180, 255), random.randint(150, 255))
            painter.setBrush(star_color)
            painter.drawEllipse(QtCore.QPointF(star_x, star_y), star_size, star_size)

        # Planets (simple examples)
        # Planet 1 (Earth-like)
        p1_size = min(width, height) * 0.2
        p1_x = width * 0.25
        p1_y = height * 0.65
        painter.setBrush(QtGui.QColor("#4A90E2")) # Blue
        painter.drawEllipse(QtCore.QRectF(p1_x - p1_size/2, p1_y - p1_size/2, p1_size, p1_size))
        painter.setBrush(QtGui.QColor("#50E3C2")) # Greenish continents
        painter.drawEllipse(QtCore.QRectF(p1_x - p1_size*0.2, p1_y - p1_size*0.15, p1_size*0.4, p1_size*0.3))
        painter.drawEllipse(QtCore.QRectF(p1_x + p1_size*0.05, p1_y + p1_size*0.1, p1_size*0.3, p1_size*0.25))

        # Planet 2 (Mars-like)
        p2_size = min(width, height) * 0.12
        p2_x = width * 0.75
        p2_y = height * 0.3
        painter.setBrush(QtGui.QColor("#D0021B")) # Red
        painter.drawEllipse(QtCore.QRectF(p2_x - p2_size/2, p2_y - p2_size/2, p2_size, p2_size))

        # Planet 3 (Saturn-like with rings)
        p3_body_size = min(width, height) * 0.15
        p3_x = width * 0.65
        p3_y = height * 0.75
        painter.setBrush(QtGui.QColor("#F8E71C")) # Yellowish body
        painter.drawEllipse(QtCore.QRectF(p3_x - p3_body_size/2, p3_y - p3_body_size/2, p3_body_size, p3_body_size))
        painter.setPen(QtGui.QPen(QtGui.QColor("#D8C0A8"), 3)) # Ring color
        painter.setBrush(Qt.NoBrush)
        painter.drawEllipse(QtCore.QRectF(p3_x - p3_body_size, p3_y - p3_body_size*0.4, p3_body_size*2, p3_body_size*0.8))

        # Text
        # Dynamic font sizes based on screen height
        title_font_size = max(18, int(height * 0.055)) # Min size 18pt, scales with height # noqa: E501
        subtitle_font_size = max(12, int(height * 0.03)) # Min size 12pt

        title_font = QtGui.QFont(QT_FLUTTER_FONT_FAMILY, title_font_size, QtGui.QFont.Bold)
        subtitle_font = QtGui.QFont(QT_FLUTTER_FONT_FAMILY, subtitle_font_size)

        painter.setFont(title_font)
        painter.setPen(QtGui.QColor("#E0FFFF")) # Light Cyan
        title_text = "Welcome to Flutter Earth 🚀" # Updated title
        title_rect = painter.fontMetrics().boundingRect(title_text)
        painter.drawText(QtCore.QRectF((width - title_rect.width()) / 2, height * 0.15, title_rect.width(), title_rect.height()), title_text) # Adjusted Y position

        painter.setFont(subtitle_font)
        painter.setPen(QtGui.QColor("#F0F8FF")) # Alice Blue
        subtitle_text1 = "See the World. 🌍"  # Updated subtitle
        subtitle_text2 = "Download the World. 🛰️" # Updated subtitle # noqa: E501
        sub1_rect = painter.fontMetrics().boundingRect(subtitle_text1)
        sub2_rect = painter.fontMetrics().boundingRect(subtitle_text2)
        painter.drawText(QtCore.QRectF((width - sub1_rect.width()) / 2, height * 0.25, sub1_rect.width(), sub1_rect.height()), subtitle_text1) # Adjusted Y
        painter.drawText(QtCore.QRectF((width - sub2_rect.width()) / 2, height * 0.25 + sub1_rect.height() + int(height*0.01), sub2_rect.width(), sub2_rect.height()), subtitle_text2) # Dynamic spacing # noqa: E501

def draw_flowers_splash(painter, width, height):
    # Ensure painter is valid
    if not painter or not painter.isActive():
        logging.error("draw_flowers_splash: Painter is not active or invalid.")
        return
    # Background: Sky blue to light green gradient
    flower_gradient = QtGui.QLinearGradient(0, 0, 0, height)
    flower_gradient.setColorAt(0, QtGui.QColor("#87CEEB")) # Sky Blue
    flower_gradient.setColorAt(0.6, QtGui.QColor("#90EE90")) # Light Green
    flower_gradient.setColorAt(1, QtGui.QColor("#3CB371")) # Medium Sea Green (grass)
    painter.fillRect(0, 0, width, height, flower_gradient)

    # Flowers
    # Enhanced colors and variety
    # Flower center colors: deep yellow, orange, brown
    center_colors_list = [QtGui.QColor("#FFD700"), QtGui.QColor("#FFA500"), QtGui.QColor("#A52A2A")] 
    # Petal colors: pink, light yellow, light blue, lavender, white
    petal_colors_list = [QtGui.QColor("#FFC0CB"), QtGui.QColor("#FFFFE0"), QtGui.QColor("#ADD8E6"), QtGui.QColor("#E6E6FA"), QtGui.QColor("#FFFFFF")]

    for i in range(random.randint(45, 75)): # Vary number of flowers
        center_x = random.randint(int(width*0.05), int(width*0.95))
        center_y = random.randint(int(height*0.45), int(height*0.98)) # Flowers mostly in lower half, bit higher start
        
        # Vary flower size more significantly
        base_flower_size = random.uniform(min(width,height)*0.015, min(width,height)*0.09)
        
        # Randomly pick center and petal colors
        current_center_color = random.choice(center_colors_list)
        current_petal_color = random.choice(petal_colors_list)
        # Ensure petal and center are not too similar (basic check, can be improved)
        if current_petal_color.name() == current_center_color.name():
            current_petal_color = random.choice([c for c in petal_colors_list if c.name() != current_center_color.name()] or [QtGui.QColor("#FFFFFF")])


        # Stem
        stem_thickness = max(1.5, base_flower_size*0.08) # Slightly thinner stems possible
        painter.setPen(QtGui.QPen(QtGui.QColor("#228B22"), stem_thickness)) # Forest Green
        # Stem goes from below flower center towards bottom of screen, with slight bend
        painter.drawLine(center_x, center_y + base_flower_size*0.25, center_x + random.randint(-int(base_flower_size*0.5), int(base_flower_size*0.5)), height + 10) # Ensure stem reaches bottom

        # Petals (simple ovals)
        painter.setBrush(current_petal_color)
        painter.setPen(Qt.NoPen) # No border for petals for a softer look
        num_petals = random.randint(5, 9) # Vary number of petals
        
        # Petal dimensions relative to flower size
        petal_width = base_flower_size * random.uniform(0.35, 0.45) 
        petal_height = base_flower_size * random.uniform(0.65, 0.75)
        
        for j in range(num_petals):
            angle = (360 / num_petals) * j + random.uniform(-5, 5) # Slight angle variation
            painter.save()
            painter.translate(center_x, center_y)
            painter.rotate(angle)
            # Draw ellipse slightly offset from center to form distinct petals
            painter.drawEllipse(QtCore.QRectF(-petal_width/2, -petal_height*0.75, petal_width, petal_height))
            painter.restore()

        # Center of the flower
        painter.setBrush(current_center_color)
        # Center size relative to flower size
        center_radius_factor = random.uniform(0.22, 0.28)
        center_diameter = base_flower_size * center_radius_factor * 2
        painter.drawEllipse(QtCore.QRectF(center_x - center_diameter/2, center_y - center_diameter/2, center_diameter, center_diameter))

    # Text
    title_font_size = max(18, int(height * 0.06)) # Min size 18pt
    subtitle_font_size = max(12, int(height * 0.032)) # Min size 12pt

    title_font = QtGui.QFont(QT_FLUTTER_FONT_FAMILY, title_font_size, QtGui.QFont.Bold)
    subtitle_font = QtGui.QFont(QT_FLUTTER_FONT_FAMILY, subtitle_font_size)

    painter.setFont(title_font)
    painter.setPen(QtGui.QColor("#2F4F4F")) # Dark Slate Gray
    title_text = "Welcome to Flutter Earth 🌻" # Updated title
    title_rect = painter.fontMetrics().boundingRect(title_text)
    painter.drawText(QtCore.QRectF((width - title_rect.width()) / 2, height * 0.2, title_rect.width(), title_rect.height()), title_text) # Adjusted Y

    painter.setFont(subtitle_font)
    painter.setPen(QtGui.QColor("#006400")) # Dark Green
    subtitle_text1 = "See the World. 🌷"  # Updated subtitle
    subtitle_text2 = "Download the World. 🌳" # Updated subtitle
    sub1_rect = painter.fontMetrics().boundingRect(subtitle_text1)
    sub2_rect = painter.fontMetrics().boundingRect(subtitle_text2)
    painter.drawText(QtCore.QRectF((width - sub1_rect.width()) / 2, height * 0.3, sub1_rect.width(), sub1_rect.height()), subtitle_text1) # Adjusted Y
    painter.drawText(QtCore.QRectF((width - sub2_rect.width()) / 2, height * 0.3 + sub1_rect.height() + int(height*0.01), sub2_rect.width(), sub2_rect.height()), subtitle_text2) # Dynamic spacing

# def draw_desert_splash(painter, width, height):
    #     # Ensure painter is valid
    #     if not painter or not painter.isActive():
    #         logging.error("draw_desert_splash: Painter is not active or invalid.")
    #         return
    #     # Background: Desert sky and sand
    #     sky_gradient = QtGui.QLinearGradient(0, 0, 0, height * 0.6)
    #     sky_gradient.setColorAt(0, QtGui.QColor("#FDB813")) # Orange-Yellow (Sunset)
    #     sky_gradient.setColorAt(1, QtGui.QColor("#FFDAB9")) # Peach Puff (Lighter sky)
    #     painter.fillRect(0, 0, width, int(height * 0.65), sky_gradient)
    #
    #     sand_gradient = QtGui.QLinearGradient(0, height * 0.6, 0, height)
    #     sand_gradient.setColorAt(0, QtGui.QColor("#F4A460")) # Sandy Brown
    #     sand_gradient.setColorAt(1, QtGui.QColor("#D2B48C")) # Tan
    #     painter.fillRect(0, int(height * 0.6), width, int(height * 0.4), sand_gradient)
    #
    #     # Sun
    #     sun_size = min(width, height) * 0.15
    #     painter.setBrush(QtGui.QColor("#FF8C00")) # Dark Orange Sun
    #     painter.drawEllipse(QtCore.QRectF(width * 0.75, height * 0.15, sun_size, sun_size))
    #
    #     # Dunes (simple wavy paths)
    #     painter.setPen(Qt.NoPen)
    #     dune_colors = [QtGui.QColor("#CD853F"), QtGui.QColor("#DEB887"), QtGui.QColor("#E0A96D")]
    #     for i in range(3):
    #         path = QtGui.QPainterPath()
    #         path.moveTo(0, height * (0.65 + i * 0.05) + random.randint(-10,10))
    #         for x in range(0, width + 50, 50):
    #             path.quadTo(x + random.randint(20,30), height * (0.6 + i * 0.06) + random.randint(-20,20) - i*10,
    #                         x + 50, height * (0.65 + i * 0.05) + random.randint(-10,10))
    #         path.lineTo(width, height)
    #         path.lineTo(0, height)
    #         path.closeSubpath()
    #         painter.setBrush(dune_colors[i % len(dune_colors)].darker(100 + i*10))
    #         painter.drawPath(path)
    #
    #     # Text
    #     title_font_size = max(18, int(height * 0.062)) # Min size 18pt
    #     subtitle_font_size = max(12, int(height * 0.033)) # Min size 12pt
    #
    #     title_font = QtGui.QFont(QT_FLUTTER_FONT_FAMILY, title_font_size, QtGui.QFont.Bold)
    #     subtitle_font = QtGui.QFont(QT_FLUTTER_FONT_FAMILY, subtitle_font_size)
    #
    #     painter.setFont(title_font)
    #     painter.setPen(QtGui.QColor("#8B4513")) # Saddle Brown
    #     title_text = "Welcome to Flutter Earth 🏜️" # Updated title
    #     title_rect = painter.fontMetrics().boundingRect(title_text)
    #     painter.drawText(QtCore.QRectF((width - title_rect.width()) / 2, height * 0.18, title_rect.width(), title_rect.height()), title_text) # Adjusted Y
    #
    #     painter.setFont(subtitle_font)
    #     painter.setPen(QtGui.QColor("#A0522D")) # Sienna
    #     subtitle_text1 = "See the World. ☀️"  # Updated subtitle
    #     subtitle_text2 = "Download the World. 🗺️" # Updated subtitle
    #     sub1_rect = painter.fontMetrics().boundingRect(subtitle_text1)
    #     sub2_rect = painter.fontMetrics().boundingRect(subtitle_text2)
    #     painter.drawText(QtCore.QRectF((width - sub1_rect.width()) / 2, height * 0.28, sub1_rect.width(), sub1_rect.height()), subtitle_text1) # Adjusted Y
    #     painter.drawText(QtCore.QRectF((width - sub2_rect.width()) / 2, height * 0.28 + sub1_rect.height() + int(height*0.01), sub2_rect.width(), sub2_rect.height()), subtitle_text2) # Dynamic spacing

    # --- Create and Show Splash Screen ---
    screen_rect = app.primaryScreen().geometry()
    splash_pixmap_width = screen_rect.width()
    splash_pixmap_height = screen_rect.height()
    splash_pixmap = QtGui.QPixmap(splash_pixmap_width, splash_pixmap_height)
    # No need to fill transparent if drawing functions fill the whole pixmap

    painter = QtGui.QPainter(splash_pixmap)
    painter.setRenderHint(QtGui.QPainter.Antialiasing)

    # --- Simplified Splash Screen Drawing ---
    # Bypassing custom drawing functions for maximum stability.
    # Draw a simple background color and text.
    try:
        # Use the current theme's main background color
        current_theme_colors_for_splash = THEMES.get(APP_CONFIG.get('theme', DEFAULT_THEME_QT), FLUTTERSHY_COLORS)
        bg_color_splash = QtGui.QColor(current_theme_colors_for_splash.get("BG_MAIN", "#FFFFFF")) # Default to white
        text_color_splash = QtGui.QColor(current_theme_colors_for_splash.get("TEXT_COLOR", "#000000")) # Default to black

        painter.fillRect(0, 0, splash_pixmap_width, splash_pixmap_height, bg_color_splash)

        painter.setPen(text_color_splash)
        painter.setFont(QtGui.QFont(QT_FLUTTER_FONT_FAMILY, 24, QtGui.QFont.Bold))
        painter.drawText(QtCore.QRectF(0, 0, splash_pixmap_width, splash_pixmap_height), Qt.AlignCenter, "Flutter Earth - Loading...")

    except Exception as e_simple_splash_draw:
        logging.error(f"Error drawing simple splash screen: {e_simple_splash_draw}", exc_info=True)
        # Fallback to a solid grey if even the simple drawing fails
        try:
            painter.fillRect(0, 0, splash_pixmap_width, splash_pixmap_height, QtGui.QColor("grey"))
            painter.setPen(QtGui.QColor("white"))
            painter.setFont(QtGui.QFont("Arial", 20))
            painter.drawText(QtCore.QRectF(0,0, splash_pixmap_width, splash_pixmap_height), Qt.AlignCenter, "Error loading splash screen.")
        except Exception as e_fallback_draw:
            logging.error(f"Error drawing fallback splash screen: {e_fallback_draw}")
            # If even this fails, the pixmap might be corrupted or painter unusable.

    painter.end()
    splash = QtWidgets.QSplashScreen(splash_pixmap)
    splash.setWindowFlags(Qt.SplashScreen | Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint) # Ensure it's frameless and on top
    splash.show()
    app.processEvents()
    ...
    def load_app_config_qt():
        ...
        if os.path.exists(config_full_path):
            try:
                ...
                default_cfg_copy['splash_screen_style'] = loaded_config.get('splash_screen_style', DEFAULT_SPLASH_SCREEN_STYLE)
            ...
            APP_CONFIG['splash_screen_style'] = DEFAULT_SPLASH_SCREEN_STYLE # Ensure default splash on first run
    ...
    class SampleDownloadThread(QtCore.QThread):
        ...
        def run(self):
            ...
            original_use_best_res = None # Initialize for finally block
            original_target_res = None   # Initialize for finally block
            ...
            try:
                if not (ee.data._credentials if hasattr(ee, 'data') and hasattr(ee.data, '_credentials') else False): # Check EE init status
                    ...
                    # No APP_CONFIG changes made yet, so no restoration needed in finally for this path
                ...
                # Capture original APP_CONFIG values before modification
                ...
                download_successful = download_manager.download_tile( mosaic_image_for_download, self.config["bbox"], output_filepath, 0, sensor_name )
                ...
                # Restore APP_CONFIG settings if they were captured and potentially modified
    ...
    # This function is used to generate the Qt stylesheet based on the selected theme colors.
    ...
    # Appearance Menu (replaces old Themes menu)
    appearance_menu_text = FLUTTERSHY_TEXTS.get("appearance_menu_text", "&Appearance ✨")
    appearance_menu = menu_bar.addMenu(appearance_menu_text)

    # Color Scheme Submenu
    color_scheme_submenu = appearance_menu.addMenu("Color Scheme")
    color_theme_action_group = QtWidgets.QActionGroup(main_window)
    color_theme_action_group.setExclusive(True)

    fluttershy_color_action = QtWidgets.QAction("Fluttershy (Light)", main_window, checkable=True)
    fluttershy_color_action.triggered.connect(lambda: apply_theme_qt("Fluttershy"))
    color_scheme_submenu.addAction(fluttershy_color_action)
    color_theme_action_group.addAction(fluttershy_color_action)

    night_mode_color_action = QtWidgets.QAction("Night Mode", main_window, checkable=True)
    night_mode_color_action.triggered.connect(lambda: apply_theme_qt("Night Mode"))
    color_scheme_submenu.addAction(night_mode_color_action)
    color_theme_action_group.addAction(night_mode_color_action)

    # Splash Screen Style Submenu
    splash_style_submenu_text = FLUTTERSHY_TEXTS.get("splash_screen_style_submenu_text", "Splash Screen Style 🌠")
    splash_style_submenu = appearance_menu.addMenu(splash_style_submenu_text)
    splash_style_action_group = QtWidgets.QActionGroup(main_window)
    splash_style_action_group.setExclusive(True)

    for style_name_key in SPLASH_SCREEN_STYLES.keys():
        action = QtWidgets.QAction(style_name_key, main_window, checkable=True)
        # Use a lambda to capture the style_name_key for the connection
        action.triggered.connect(lambda checked, name=style_name_key: set_splash_screen_preference(name))
        splash_style_submenu.addAction(action)
        splash_style_action_group.addAction(action)
    ...
    tools_menu.addAction(vector_dl_action) # Add Vector Download action to Tools menu
    ...
    def browse_output_dir_qt():
        ...
        if directory:
            ...
            APP_CONFIG['output_dir'] = directory # Update APP_CONFIG directly
    ...
    def apply_theme_qt(theme_name):
        ...
        APP_CONFIG['theme'] = theme_name # Update internal config state
        # No need to call update_and_save_app_config_qt() here, it will be called by the action trigger if needed
    ...
    # Connect color theme actions (defined during menu creation)
    # fluttershy_color_action.triggered.connect(lambda: apply_theme_qt("Fluttershy"))
    # night_mode_color_action.triggered.connect(lambda: apply_theme_qt("Night Mode"))
    # The connect was already done when actions were created.
    ...
    def update_and_save_app_config_qt():
        ...
        # APP_CONFIG['splash_screen_style'] is updated by its own handler
    ...
    def update_theme_menu_checked_state():
        ...
        for action in color_theme_action_group.actions():
            if action.text() == current_theme_name or \
               (action.text() == "Fluttershy (Light)" and current_theme_name == "Fluttershy"): # Handle display name difference
                action.setChecked(True)
                break
        
        current_splash_pref = APP_CONFIG.get('splash_screen_style', DEFAULT_SPLASH_SCREEN_STYLE)
        for action in splash_style_action_group.actions():
            if action.text() == current_splash_pref:
                action.setChecked(True)
                break

    def set_splash_screen_preference(style_key_name):
        global APP_CONFIG
        if style_key_name in SPLASH_SCREEN_STYLES:
            APP_CONFIG['splash_screen_style'] = style_key_name
            update_and_save_app_config_qt() # Save the change
            logging.info(f"Splash screen style preference set to: {style_key_name}")
            QtWidgets.QMessageBox.information(main_window, "Splash Screen Style",
                                              f"Splash screen style set to '{style_key_name}'.\n"
                                              "This will take effect on the next application start.")
    ...
    def start_download_qt():
        ...
        try:
            ...
            setup_main_app_logging(params_for_thread['output_dir'])
    ...
    def on_sample_download_finished(success, message_or_filepath):
        ...
        if success:
            ...
            if data_viewer_tab:
                ...
                # Ensure the correct tabs are selected to show the viewer
            ...
            splash_msg_text = f"Sample data download failed: {message_or_filepath}"
            splash_message_color = QtGui.QColor("red")
            logging.error(f"Sample data download failed: {message_or_filepath}")
    ...
    def check_and_download_sample_data_on_startup():
        ...
        if not (ee.data._credentials if hasattr(ee, 'data') and hasattr(ee.data, '_credentials') else False):
             logging.warning("EE not initialized. Skipping sample data download prompts.")
             if splash.isVisible(): splash.showMessage("EE not ready for sample data download.", Qt.AlignBottom | Qt.AlignHCenter, QtGui.QColor("orange"))
             app.processEvents()
             return

        logging.info("Checking for missing sample data...")
        if splash.isVisible(): splash.showMessage("Checking for optional sample data...", Qt.AlignBottom | Qt.AlignHCenter, QtGui.QColor(FLUTTERSHY_COLORS.get("TEXT_LIGHT", "grey")))
        app.processEvents()

        samples_to_prompt = []

        for sample_key, config in ALL_SAMPLE_CONFIGS.items():
            try:
                sample_output_dir = os.path.join(SCRIPT_BASE_PATH, config.get("output_subdir", "sample_data")) # Use default if missing
                sample_file_name = f"{config.get('area_name', sample_key)}_{config.get('sensor', 'Unknown')}_{config.get('year', 'YYYY')}-{config.get('month', 'MM'):02d}.tif"
                expected_sample_filepath = os.path.join(sample_output_dir, sample_file_name)

                if os.path.exists(expected_sample_filepath):
                    logging.info(f"Sample data found for '{sample_key}': {expected_sample_filepath}")
                    # Optionally load the default sample if it exists
                    if sample_key == DEFAULT_SAMPLE_KEY and data_viewer_tab:
                         # Defer the initial loading to allow the main event loop to start
                         QtCore.QTimer.singleShot(100, lambda path=expected_sample_filepath: data_viewer_tab.load_specific_raster(filepath=path))
                         logging.info(f"Scheduled initial load of default sample '{sample_key}' for DataViewerTab.")
                         if main_tab_widget and post_processing_main_tab and post_processing_sub_tabs: # Switch to viewer
                             main_tab_widget.setCurrentWidget(post_processing_main_tab)
                             post_processing_sub_tabs.setCurrentWidget(data_viewer_tab)
                else:
                    logging.info(f"Sample data missing for '{sample_key}': {expected_sample_filepath}")
                    samples_to_prompt.append((sample_key, config, expected_sample_filepath))
            except Exception as e:
                logging.error(f"Error checking sample config for '{sample_key}': {e}", exc_info=True)
                # Continue to check other samples

        if not samples_to_prompt:
            logging.info("All sample data found or no samples defined.")
            if splash.isVisible(): splash.showMessage("All sample data found.", Qt.AlignBottom | Qt.AlignHCenter, QtGui.QColor(FLUTTERSHY_COLORS.get("TEXT_LIGHT", "grey")))
            app.processEvents()
            return

        # Prompt for each missing sample individually
        for sample_key, config, expected_sample_filepath in samples_to_prompt:
            reply = QtWidgets.QMessageBox.question(main_window, f"Optional Sample Data ({config.get('area_name', sample_key)})",
                                                   f"The sample dataset for '{config.get('area_name', sample_key)}' is missing.\n\n"
                                                   "This is optional and allows you to test processing features.\n\n"
                                                   f"Would you like to download the '{config.get('area_name', sample_key)}' sample now? (Requires internet & EE auth). Check logs for progress.",
                                                   QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
                                                   QtWidgets.QMessageBox.No)

            if reply == QtWidgets.QMessageBox.Yes:
                logging.info(f"User opted to download sample data for key '{sample_key}'. Attempting download to: {expected_sample_filepath}")
                if splash.isVisible(): splash.showMessage(f"Downloading sample data ({config.get('area_name', sample_key)})...", Qt.AlignBottom | Qt.AlignHCenter, QtGui.QColor(FLUTTERSHY_TEXTS.get("TEXT_LIGHT", "grey")))
                app.processEvents()

                # Start download thread for this specific sample
                sample_download_thread_instance = SampleDownloadThread(sample_key, config, SCRIPT_BASE_PATH)
                ...
                # Note: This will start multiple threads if user says yes to multiple prompts.
                # DownloadManager handles thread pooling for the actual downloads.
            else:
                logging.info(f"User declined to download sample data for key '{sample_key}'.")
                if splash.isVisible(): splash.showMessage(f"Download skipped for sample: {config.get('area_name', sample_key)}.", Qt.AlignBottom | Qt.AlignHCenter, QtGui.QColor(FLUTTERSHY_TEXTS.get("TEXT_LIGHT", "grey")))
                app.processEvents()
    ...
    # main_window.showMaximized() # Show after splash is finished
    if splash.isVisible():
