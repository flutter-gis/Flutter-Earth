...
Rectangle {
    ...
    function showPanel(panelType) {
        if (backend && backend.setPanelState) {
            backend.setPanelState(panelType, {visible: true})
        }
    }
    ...
    Column {
        ...
        Repeater {
            model: [
                ...
                { label: qsTr("Toolbox"), panelType: "toolbox", icon: "🧰" },
                { label: qsTr("Bookmarks"), panelType: "bookmarks", icon: "🔖" },
                { label: qsTr("Attribute Table"), panelType: "attribute_table", icon: "📋" }
            ]
            delegate: Rectangle {
                width: parent.width
                height: 48
                ...
                MouseArea {
                    ...
                    onReleased: {
                        ...
                        if (modelData.panelType) {
                            sidebar.showPanel(modelData.panelType)
                        } else {
