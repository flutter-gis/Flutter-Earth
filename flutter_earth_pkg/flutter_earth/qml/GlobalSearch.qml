...
Window {
    ...
    function executeResult(result) {
        switch (result.type) {
            case "layer":
                ...
                if (backend.focusOnLayer) backend.focusOnLayer(result.id); // Conceptual backend call
                else console.warn("Backend function focusOnLayer not implemented.");
                break
                
            case "bookmark":
                console.log("Go to bookmark:", result.name);
                if (backend.goToBookmark) backend.goToBookmark(result.name); // Conceptual backend call
                else console.warn("Backend function goToBookmark not implemented.");
                break
                
            case "tool": // This is for tools found directly by backend.search
                console.log("Activate tool:", result.name);
                if (backend.activateGISTool) backend.activateGISTool(result.name); // Conceptual backend call
                else console.warn("Backend function activateGISTool not implemented.");
                break
                
            case "command": // This is for commands generated by getCommandSuggestions
    ...
    signal openSettingsRequested(); // Signal to be caught by main.qml
    signal openAddLayerDialogRequested(); // Signal for adding layer

    function executeCommand(action, data) {
        switch (action) {
            case "tool": // Command to activate a tool
                console.log("Execute command - tool:", data.tool);
                if (backend.activateGISTool) backend.activateGISTool(data.tool); // Conceptual general tool activation
                else if (backend.startMeasurement) backend.startMeasurement(data.tool); // Fallback if it's a measurement tool
                else console.warn("Backend function activateGISTool or startMeasurement not implemented for tool command.");
                break
                
            case "add_layer":
                console.log("Request Open add layer dialog");
                openAddLayerDialogRequested(); // Emit signal
                break
            ...
            case "open_settings":
                console.log("Request Open settings");
                openSettingsRequested(); // Emit signal
                break
        }
    }
